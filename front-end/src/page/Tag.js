...
function Tag({ match }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await HttpUtil.get(ApiUtil.API_GET_TAG_INFO + `${tagId}`);
        const responseTagName = await HttpUtil.get(ApiUtil.API_GET_TAG_NAME + `${tagId}`);
        if (responseTagName)
          setTagName(responseTagName);
